...
このエンドポイントがInngestサーバーに対するクライアントとなります。（ので、認証不要でアクセスできるようにしておく必要があります）
...
ここでは例として、渡された `data.name` を使って `Hello, {渡された名前}!` をコンソールに出力するという内容になっています。
...
* `uuid`：そのジョブの実行者であるユーザーのidを
* `status`：ジョブの実行状況（`waiting` `runnning` `finished` `failed` といった）を
* `context`：ジョブに渡すデータやジョブの実行結果などを
* `started_at` `ended_at`：それぞれジョブの開始・終了日時を

入れるイメージです。

これを受けて、ジョブをキューするAPIエンドポイントを以下のように修正します。
...
続いて②でSupabaseのadminクライアントを生成します。これは、`jobs` テーブルへのレコードの挿入はadminだけしかできないようにRLSで制約されていることが設計上妥当なので、その想定によるものです。

> 参考：[Supabase Javascript Client](https://supabase.com/docs/reference/javascript/admin-api)
...
まず①で `HelloEvent` 型の定義を修正してあります。（あらかじめDRYにしておいたおかげで修正箇所はここだけで済みました👌）
...
次に、Inngest側で発行された "Signing Key" というものと "Event Key" というものをVercelに環境変数として登録します。
