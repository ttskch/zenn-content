...
* `replace` プロパティの本来の使い方は以下のようなもの
    ...
    * これにより、あるパッケージが **`better/library` と `other/package` に依存していて、`other/package` が `original/library:1.0.2` に依存している場合** でも、**`original/library:1.0.2` はインストールされず、代わりに `better/library` が使われる** という挙動を実現できる
