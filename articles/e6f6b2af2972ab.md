...
グローバルでデフォルトで使いたいバージョンをリンクしておきます。
...
`pecl install` のあとは必要に応じて `php.ini` を修正します。PHP拡張によっては `pecl install` 中に `php.ini` の先頭や末尾に `extension=...` を自動で追記してくれるものもあるので要注意です。
...
次に、PHPバージョンを固定したいディレクトリの直下に、以下のような内容の `.envrc` というファイルを作成します。
...
これで、同ディレクトリ配下ではPHP 8.1が優先的に（グローバルのデフォルトはPHP 8.3であるにもかかわらず）使用されます。
