...
emoji: "🕸️"
type: "tech"
topics: ["php", "apiplatform", "laravel"]
published: true
---

# 3行で

- Laravel版の [API Platofrm](https://api-platform.com/) でSystem Processorをデコレートしようとしたら循環依存が発生した
...
- https://github.com/api-platform/laravel/blob/v4.2.2/ApiPlatformProvider.php#L457-L464

ここで `HydraLinkProcessor` または `WriteProcessor` がバインドされており、前者であった場合でも、`HydraLinkProcessor` は

- https://github.com/api-platform/laravel/blob/v4.2.2/ApiPlatformProvider.php#L453-L455
...
ここにあるとおり `CallableProcessor` が `WriteProcessor` の依存であるため、
